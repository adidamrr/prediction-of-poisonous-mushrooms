...
<img width="2940" height="1462" alt="image" src="https://github.com/user-attachments/assets/b954a5b9-ef98-4130-b65a-80deceb03397" />
...
Проект представляет собой end-to-end решение для скрининга риска токсичности грибов:
- модель на табличных данных определяет риск ядовитости;
- FastAPI-сервис отдает prediction через API;
- Streamlit-интерфейс позволяет вручную вводить признаки гриба;
- пользовательский feedback сохраняется в Postgres и используется для последующего fine-tuning.

## Результат на Kaggle
- Public Score: `0.96134`
- Private Score: `0.96142`
<img width="1152" height="629" alt="image" src="https://github.com/user-attachments/assets/f9d357b6-e351-43d6-b91a-493b982bffd9" />
...
## Быстрый старт
```bash
git clone https://github.com/adidamrr/prediction-of-poisonous-mushrooms.git
cd prediction-of-poisonous-mushrooms
...
## Готовая модель
В репозитории уже лежат готовые артефакты модели:
- `model/model.cbm`
- `model/meta.json`

Поэтому сервис можно запускать сразу, без предварительного обучения.

[train.py](/Users/grigory/Code/pet-projects/3/train.py) нужен только если хочется обучить модель заново на исходных данных конкурса.

## Данные
Папка `data/` не включена в репозиторий из-за размера файлов.

Если нужно запустить [train.py](/Users/grigory/Code/pet-projects/3/train.py), данные нужно отдельно скачать с Kaggle: [playground-series-s4e8/data](https://www.kaggle.com/competitions/playground-series-s4e8/data)
...
## Что делает сервис
Пользователь заполняет признаки гриба в UI. Frontend отправляет их в backend. Backend:
- загружает модель;
- считает prediction;
...
Если хочется обучить модель с нуля:
